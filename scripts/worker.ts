...
const worker = new DurableWorker({ concurrency: positiveInt("WORKER_CONCURRENCY", 2), pollMs: positiveInt("WORKER_POLL_MS", 1_000), leaseMs: positiveInt("WORKER_LEASE_MS", 30_000) });
...
const server = createServer(async (request, response) => {
  ...
  if (request.url === "/health/live") return response.end(JSON.stringify({ status: "vivo", service: "worker", activeJobs: worker.activeCount }));
