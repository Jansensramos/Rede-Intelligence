...
async function main() {
  ...
  const dataIntelligenceDemo = await seedDataIntelligenceDemo(context, butantaStudy.projectId);

  const marketProductDemo = await seedMarketProductDemo(prisma, {
    organizationId: organization.id,
    userId: user.id,
    projectId: butantaStudy.projectId,
    landAssetId: legalLandStudy.landAssetId,
    landStudyId: legalLandWorkspace.landStudyId,
  });
