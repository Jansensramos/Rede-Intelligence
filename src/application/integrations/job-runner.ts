  ...
  const rows = await prisma.$queryRaw<IntegrationJob[]>`
    ...
    WHERE id = (
      ...
      WHERE status NOT IN ('SUCCEEDED', 'CANCELLED', 'DEAD_LETTER')
        AND job_type IN ('SYNC_INSTALLATION', 'POLL_INSTALLATION', 'DELIVER_WEBHOOKS', 'PROCESS_DESIGN_FILE', 'PROCESS_SIGNATURE_WEBHOOK', 'SEND_TRANSACTIONAL_EMAIL', 'FINANCIAL_PROVIDER', 'ENTERPRISE_SYNC')
