...
export async function getAccountingWorkspace(context: Pick<AccountingContext, "organizationId" | "role">, projectId: string) {
  ...
  return {
    ...
    permissions: { canPost: hasAccountingCapability(context.role, "ACCOUNTING_ENTRY_POST"), canReview: hasAccountingCapability(context.role, "ACCOUNTING_ENTRY_REVIEW"), canReverse: hasAccountingCapability(context.role, "ACCOUNTING_REVERSE"), canClose: hasAccountingCapability(context.role, "ACCOUNTING_CLOSE"), canReopen: hasAccountingCapability(context.role, "ACCOUNTING_REOPEN"), canManageTax: hasAccountingCapability(context.role, "TAX_MANAGE"), canConsolidate: hasAccountingCapability(context.role, "CONSOLIDATION_MANAGE") },
