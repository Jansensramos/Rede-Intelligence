...
export function ClosureOperabilityPanel({ projectId, latest, gate, canWrite, canApprove, canReopen }: { projectId: string; latest: LatestClosure; gate: Gate; canWrite: boolean; canApprove: boolean; canReopen: boolean }) {
  ...
  return <section className="panel">
    ...
    {gate.overall !== "APTO" && <div className="panel-actions" style={{ marginTop: 12, flexWrap: "wrap" }}>
      {!["READY","OK","APTO"].includes(gate.operational.status) && <Link className="button button-secondary" href="/engenharia-obra">Tratar pendência operacional</Link>}
      {!["READY","OK","APTO"].includes(gate.contractual.status) && <Link className="button button-secondary" href="/suprimentos">Tratar pendência contratual</Link>}
      {!["READY","OK","APTO"].includes(gate.legal.status) && <Link className="button button-secondary" href="/juridico">Tratar pendência jurídica</Link>}
      {!["READY","OK","APTO"].includes(gate.financial.status) && <Link className="button button-secondary" href="/financeiro">Tratar pendência financeira</Link>}
      {!["READY","OK","APTO"].includes(gate.accounting.status) && <Link className="button button-secondary" href="/contabilidade-controladoria">Tratar pendência contábil</Link>}
    </div>}
