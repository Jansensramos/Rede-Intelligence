...
export function SalesOperabilityPanel({ workspace, canWrite, canApprove }: { workspace: SalesWorkspaceView; canWrite: boolean; canApprove: boolean }) {
  ...
  return (
    <section className="panel">
      ...
      {!canWrite && !canApprove && <div className={styles.notice}>Seu perfil está em modo de leitura nesta área. As operações comerciais permanecem visíveis, mas as ações de alteração e aprovação estão desabilitadas.</div>}
      ...
      <div className={styles.toolbar}><div className={styles.toolbarActions}>
        <button className="button button-secondary" disabled={pending || !canWrite} onClick={() => setModal({ type: "customer" })}><UserPlus size={15}/> Novo cliente</button>
        <button className="button button-secondary" disabled={pending || !canWrite} onClick={() => setModal({ type: "lead" })}><ContactRound size={15}/> Novo lead</button>
        <button className="button button-secondary" disabled={pending || !canWrite || !activeTable} onClick={() => setModal({ type: "proposal" })}><BadgeDollarSign size={15}/> Nova proposta</button>
        <button className="button button-secondary" disabled={pending || !canWrite} onClick={() => setModal({ type: "reservation" })}><CalendarCheck2 size={15}/> Nova reserva</button>
      </div><button className="button button-primary" disabled={pending || !canWrite || !activeTable} onClick={() => setModal({ type: "sale" })}><ShoppingCart size={15}/> Registrar venda</button></div>
      ...
      <div className="data-table-scroll" style={{ marginTop: 18 }}><table className="data-table"><thead><tr><th>Etapa</th><th>Registro</th><th>Situação</th><th>Próxima ação</th></tr></thead><tbody>
        {openLeads.slice(0, 8).map((lead) => <tr key={`lead-${lead.id}`}><td>Lead</td><td><strong>{lead.name}</strong><small>Origem: {lead.source}</small></td><td className={styles.statusCell}>{lead.stage.replaceAll("_", " ")}</td><td><button className="button button-secondary" disabled={pending || !canWrite || workspace.customers.length === 0} onClick={() => setModal({ type: "convertLead", leadId: lead.id, name: lead.name })}>Converter em cliente</button></td></tr>)}
        {pendingReservations.slice(0, 8).map((reservation) => <tr key={reservation.id}><td>Reserva</td><td>{reservation.unit} · {reservation.customer}</td><td className={styles.statusCell}>{RESERVATION_STATUS[reservation.status] ?? reservation.status}</td><td><div className="panel-actions">{reservation.status === "ACTIVE" && <button className="button button-secondary" disabled={pending || !canWrite} onClick={() => run(() => confirmSalesReservationAction(reservation.id), "Reserva confirmada.")}>Confirmar</button>}<button className="button button-secondary" disabled={pending || !canWrite} onClick={() => setModal({ type: "release", reservationId: reservation.id, label: `${reservation.unit} · ${reservation.customer}` })}>Liberar</button></div></td></tr>)}
        {draftSales.slice(0, 8).map((sale) => <tr key={sale.id}><td>Venda</td><td>{sale.unit} · {sale.buyers.join(", ")}</td><td className={styles.statusCell}>{SALE_STATUS[sale.status] ?? sale.status}</td><td><button className="button button-primary" disabled={pending || !canApprove} onClick={() => setModal({ type: "approve", saleId: sale.id, unit: sale.unit, soldPrice: Number(sale.soldPrice) })}><FileCheck2 size={15}/> Aprovar e montar plano</button></td></tr>)}
        {approvedSales.slice(0, 12).map((sale) => <tr key={`approved-${sale.id}`}><td>Contrato</td><td><strong>{sale.unit}</strong> · {sale.buyers.join(", ")}<small>{sale.contractNumber ? `Contrato ${sale.contractNumber} · ` : ""}{sale.installments} parcela(s) · recebido {currency.format(Number(sale.received))}</small></td><td className={styles.statusCell}>Aprovada</td><td><div className="panel-actions"><button className="button button-secondary" disabled={pending || !canApprove || Number(sale.received) >= Number(sale.soldPrice)} onClick={() => setModal({ type: "renegotiate", saleId: sale.id, unit: sale.unit, soldPrice: Number(sale.soldPrice), received: Number(sale.received) })}>Renegociar</button><button className="button button-secondary" disabled={pending || !canApprove} onClick={() => setModal({ type: "rescind", saleId: sale.id, unit: sale.unit, soldPrice: Number(sale.soldPrice), received: Number(sale.received) })}>Distrato</button></div></td></tr>)}
