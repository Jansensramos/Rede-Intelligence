...
export function PeopleOperabilityPanel({ workspace }: { workspace: PeoplePerformanceWorkspaceView }) {
  ...
  return <section className="panel">
    ...
    <div className="data-table-scroll"><table className="data-table"><thead><tr><th>Desvio</th><th>Situação</th><th>Investigação</th><th>Ação</th></tr></thead><tbody>{workspace.varianceCases.map((item) => {
      ...
      return <tr key={item.id}><td><strong>{item.code} · {item.title}</strong></td><td>
        ...
        <div className="panel-actions">
          ...
          {item.status === "CLASSIFIED" && <><button className="text-button" disabled={pending || !workspace.permissions.canManageRootCause || !allocationComplete} title={!allocationComplete ? "A alocação das causas deve totalizar 100% e estar validada." : undefined} onClick={() => run(() => transitionPerformanceVarianceAction(item.id, "VALIDATED"), "Desvio validado.")}>Validar</button><button className="text-button" disabled={pending || !workspace.permissions.canManageRootCause} onClick={() => run(() => transitionPerformanceVarianceAction(item.id, "UNDER_ANALYSIS"), "Desvio reaberto para análise.")}>Reabrir análise</button></>}
          {item.status === "VALIDATED" && <><button className="text-button" disabled={pending || !workspace.permissions.canManageRootCause || !dependenciesClosed || !actionsClosed} title={!dependenciesClosed ? "Resolva ou cancele todas as dependências externas." : !actionsClosed ? "Verifique ou cancele todas as ações corretivas." : undefined} onClick={() => run(() => transitionPerformanceVarianceAction(item.id, "CLOSED"), "Desvio encerrado.")}>Encerrar</button><button className="text-button" disabled={pending || !workspace.permissions.canManageRootCause} onClick={() => run(() => transitionPerformanceVarianceAction(item.id, "UNDER_ANALYSIS"), "Desvio reaberto para análise.")}>Reabrir análise</button></>}
        </div>
      </td><td>
    ...
    {actions.length > 0 && <div className="data-table-scroll" style={{ marginTop: 18 }}><table className="data-table"><thead><tr><th>Ação corretiva</th><th>Prioridade</th><th>Situação</th><th>Próxima etapa</th></tr></thead><tbody>{actions.map((action) => <tr key={action.id}><td><strong>{action.title}</strong><small>{action.evidence} evidência(s)</small></td><td>{PRIORITY[action.priority] ?? action.priority}</td><td>{ACTION_STATUS[action.status] ?? action.status}</td><td><div className="panel-actions">
      ...
      {action.status === "ACTIVE" && <><button className="button button-secondary" disabled={pending || !workspace.permissions.canManageRootCause} onClick={() => run(() => transitionCorrectiveActionAction(action.id, "COMPLETED"), "Ação concluída.")}>Concluir</button><button className="text-button" disabled={pending || !workspace.permissions.canManageRootCause} onClick={() => run(() => transitionCorrectiveActionAction(action.id, "BLOCKED"), "Ação marcada como bloqueada.")}>Bloquear</button></>}
      {action.status === "BLOCKED" && <button className="button button-secondary" disabled={pending || !workspace.permissions.canApproveAction} onClick={() => run(() => transitionCorrectiveActionAction(action.id, "ACTIVE"), "Ação retomada.")}>Retomar</button>}
      ...
      {["DRAFT","PENDING_APPROVAL","ACTIVE","BLOCKED"].includes(action.status) && <button className="text-button" disabled={pending || !workspace.permissions.canManageRootCause} onClick={() => run(() => transitionCorrectiveActionAction(action.id, "CANCELLED"), "Ação cancelada.")}>Cancelar</button>}
