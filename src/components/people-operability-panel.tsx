...
export function PeopleOperabilityPanel({ workspace }: { workspace: PeoplePerformanceWorkspaceView }) {
  ...
  return <section className="panel">
    ...
    <div className="data-table-scroll"><table className="data-table"><thead><tr><th>Desvio</th><th>Situação</th><th>Investigação</th><th>Ação</th></tr></thead><tbody>{workspace.varianceCases.map((item) => <tr key={item.id}><td><strong>{item.code} · {item.title}</strong></td><td>{item.status}</td><td>{item.investigation ? item.investigation.problemStatement : "Não iniciada"}</td><td>{!item.investigation ? <button className="button button-secondary" disabled={pending} onClick={() => { const problem = prompt("Problema a investigar:"); if (problem) act(() => startRootCauseInvestigationAction(item.id, problem)); }}>Investigar causa</button> : <div className="panel-actions"><button className="button button-secondary" disabled={pending} onClick={() => { const description = prompt("Hipótese causal:"); if (description) act(() => addCausalHypothesisAction(item.investigation!.id, "OTHER", description)); }}>Hipótese</button><button className="button button-secondary" disabled={pending} onClick={() => { const title = prompt("Título da ação corretiva:"); if (!title) return; const description = prompt("Descrição da ação:"); if (description) act(() => createCorrectiveActionAction(item.investigation!.id, title, description, "MEDIUM")); }}>Nova ação</button></div>}</td></tr>)}{workspace.varianceCases.length === 0 && <tr><td colSpan={4}>Nenhum desvio registrado. Os desvios são gerados pela análise de eficiência.</td></tr>}</tbody></table></div>
    {workspace.varianceCases.flatMap((item) => item.investigation?.actions ?? []).length > 0 && <div className="data-table-scroll"><table className="data-table"><thead><tr><th>Ação corretiva</th><th>Prioridade</th><th>Situação</th><th>Próxima etapa</th></tr></thead><tbody>{workspace.varianceCases.flatMap((item) => item.investigation?.actions ?? []).map((action) => <tr key={action.id}><td>{action.title}</td><td>{action.priority}</td><td>{action.status}</td><td>{action.status === "DRAFT" && <button className="button button-secondary" disabled={pending} onClick={() => act(() => transitionCorrectiveActionAction(action.id, "PENDING_APPROVAL"))}>Enviar para aprovação</button>}{action.status === "PENDING_APPROVAL" && <button className="button button-secondary" disabled={pending} onClick={() => act(() => transitionCorrectiveActionAction(action.id, "ACTIVE"))}>Aprovar e ativar</button>}{action.status === "ACTIVE" && <button className="button button-secondary" disabled={pending} onClick={() => act(() => transitionCorrectiveActionAction(action.id, "COMPLETED"))}>Concluir</button>}{action.status === "COMPLETED" && <button className="button button-secondary" disabled={pending} onClick={() => act(() => transitionCorrectiveActionAction(action.id, "VERIFIED"))}>Verificar</button>}</td></tr>)}</tbody></table></div>}
  </section>;
}
