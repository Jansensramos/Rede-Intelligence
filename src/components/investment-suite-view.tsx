  ...
  return (
    <>
      <section className="command-hero">
        <div><span className="eyebrow">CENTRAL EXECUTIVA DE DECISÃO</span><h2>{workspace.title}</h2><p>Governança, risco e decisão sobre o Snapshot v{workspace.bundle.studyVersionNumber}.</p></div>
...
function StudioView({ workspace, update, setMessage, canEdit }: { workspace: InvestmentCaseWorkspace; update: (value: InvestmentCaseWorkspace) => void; setMessage: (value: string) => void; canEdit: boolean }) {
  ...
  return <>
    ...
    <section className="panel materials-panel"><div className="panel-heading"><div><span className="eyebrow">MATERIAIS GERADOS</span><h2>Registro de artefatos</h2></div><Archive size={19} /></div>{workspace.artifacts.length ? <div className="materials-table"><div className="materials-row materials-head"><span>Material</span><span>Versão</span><span>Snapshot</span><span>Status</span><span>Páginas</span><span /></div>{workspace.artifacts.map((item) => <div className="materials-row" key={item.id}><strong>{artifactLabel(item.type)}</strong><span>v{item.version}</span><span>{item.sourceOutdated ? "DESATUALIZADO" : `v${workspace.bundle.studyVersionNumber}`}</span><span className={`artifact-status artifact-${item.status.toLowerCase()}`}>{item.status}</span><span>{item.pageCount ?? "—"}</span><button onClick={() => startTransition(async () => { const response = await downloadArtifactAction(item.id); if (response.ok) downloadBase64(response.data.fileName, response.data.mimeType, response.data.contentBase64); else setMessage(response.error); })}><Download size={15} /></button></div>)}</div> : <div className="empty-state"><FileText size={18} /> Nenhum material gerado.</div>}</section>
