...
const ORDER_STATUS: Record<string, string> = {
  ...
  DELIVERED: "Entregue",
  CANCELLED: "Cancelado",
  SUPERSEDED: "Substituído por contrato",
};
  ...
  return (
    <section className={styles.section}>
      <div className={styles.header}>
        <div><span className={styles.kicker}>Formalização da compra</span><h2 className={styles.title}>Pedidos, contratos e aditivos</h2><p className={styles.description}>Transforme uma cotação decidida em documento de compra ou compromisso contratual, preservando aprovação e histórico.</p></div>
        <div className={styles.actions}>
          <button className="button button-secondary" onClick={() => setForm(form === "order" ? null : "order")}><ShoppingBag size={15} /> Novo pedido</button>
          <button className="button button-secondary" onClick={() => setForm(form === "contract" ? null : "contract")}><FileSignature size={15} /> Novo contrato</button>
          <button className="button button-primary" onClick={() => setForm(form === "amendment" ? null : "amendment")}><SquarePen size={15} /> Novo aditivo</button>
        </div>
      </div>

      <div className={styles.summaryGrid}>
        <div className={styles.summaryCard}><span className={styles.summaryLabel}>Pedidos registrados</span><span className={styles.summaryValue}>{workspace.orders.length}</span></div>
        <div className={styles.summaryCard}><span className={styles.summaryLabel}>Contratos</span><span className={styles.summaryValue}>{workspace.contracts.length}</span></div>
        <div className={styles.summaryCard}><span className={styles.summaryLabel}>Valor contratado atualizado</span><span className={styles.summaryValue}>{money(contractedValue)}</span></div>
        <div className={styles.summaryCard}><span className={styles.summaryLabel}>Cotações decididas disponíveis</span><span className={styles.summaryValue}>{decidedSources.length}</span></div>
      </div>

      {feedback && <div className={styles.info}><CheckCircle2 size={18} /><div><strong>Contratação</strong><p>{feedback}</p></div></div>}

      {(form === "order" || form === "contract") && <div className={styles.formCard}>
        <div className={styles.field}><label>Cotação decidida</label><select value={sourceQuotationId} onChange={(event) => setSourceQuotationId(event.target.value)}><option value="">Selecione a cotação</option>{decidedSources.map((item) => <option value={item.id} key={item.id}>{item.number} · {item.title} · {item.selectedProposal?.supplierName}</option>)}</select></div>
        {!operability.companyId && <div className={styles.empty}>O empreendimento precisa ter uma SPE/empresa vinculada antes da contratação.</div>}
        {selectedSource && <div className={styles.info}><FileSignature size={18} /><div><strong>{selectedSource.selectedProposal?.supplierName}</strong><p>{selectedSource.selectedProposal?.items.length} item(ns) serão importados da proposta vencedora.</p></div></div>}
      </div>}

      {form === "order" && selectedSource && <form action={submitOrder} className={styles.formCard}>
        <h3 className={styles.formTitle}>Novo pedido de compra</h3>
        <div className={styles.grid2}><div className={styles.field}><label>Número</label><input name="number" required placeholder="PC-0001" /></div><div className={styles.field}><label>Título</label><input name="title" required placeholder="Objeto do pedido" /></div></div>
        <div className={styles.field}><label>Escopo</label><textarea name="scope" required placeholder="Escopo e condições principais do pedido" /></div>
        <div className={styles.grid2}><div className={styles.field}><label>Entrega prevista</label><input name="deliveryAt" type="date" /></div><div className={styles.field}><label>Condições de pagamento</label><input name="paymentTerms" placeholder="Ex.: 30/60 dias" /></div></div>
        <div className={styles.formActions}><button className="button button-secondary" type="button" onClick={() => setForm(null)}>Cancelar</button><button className="button button-primary" disabled={busy || !operability.companyId} type="submit">Criar pedido</button></div>
      </form>}

      {form === "contract" && selectedSource && <form action={submitContract} className={styles.formCard}>
        <h3 className={styles.formTitle}>Novo contrato</h3>
        <div className={styles.grid2}><div className={styles.field}><label>Número</label><input name="number" required placeholder="CT-0001" /></div><div className={styles.field}><label>Título</label><input name="title" required placeholder="Objeto do contrato" /></div></div>
        <div className={styles.grid2}><div className={styles.field}><label>Tipo</label><select name="type" defaultValue="CONSTRUCTION"><option value="SUPPLY">Fornecimento</option><option value="SERVICE">Serviço</option><option value="CONSTRUCTION">Construção</option><option value="DESIGN">Projeto</option><option value="CONSULTING">Consultoria</option><option value="LEASE">Locação</option><option value="ACQUISITION">Aquisição</option><option value="OTHER">Outro</option></select></div><div className={styles.field}><label>Forma de faturamento</label><select name="billingModel" defaultValue="MEASUREMENT"><option value="MEASUREMENT">Medição</option><option value="FIXED_INSTALLMENT">Parcela fixa</option><option value="MONTHLY">Mensal</option><option value="MILESTONE">Marco</option><option value="DELIVERY">Entrega</option><option value="ADVANCE">Adiantamento</option><option value="CUSTOM">Personalizado</option></select></div></div>
        <div className={styles.field}><label>Escopo contratual</label><textarea name="scope" required placeholder="Escopo, entregáveis e limites do contrato" /></div>
        <div className={styles.grid2}><div className={styles.field}><label>Início</label><input name="startsAt" type="date" required /></div><div className={styles.field}><label>Término</label><input name="endsAt" type="date" required /></div></div>
        <div className={styles.field}><label>Condições de pagamento</label><input name="paymentTerms" /></div>
        <div className={styles.grid2}><div className={styles.field}><label>Retenção</label><input name="retentionRate" type="number" min="0" step="0.0001" defaultValue="0" /></div><div className={styles.field}><label>Garantia</label><input name="warrantyTerms" placeholder="Condições de garantia" /></div></div>
      ...
      <div className={styles.tableWrap}><table className={styles.table}><thead><tr><th>Tipo</th><th>Registro</th><th>Valor</th><th>Situação</th><th>Próxima ação</th></tr></thead><tbody>
        {workspace.orders.filter((item) => ["DRAFT", "IN_APPROVAL"].includes(item.status)).map((item) => <tr key={`order-${item.id}`}><td>Pedido</td><td><span className={styles.recordTitle}>{item.number}</span><span className={styles.recordMeta}>{item.title} · {item.supplier}</span></td><td>{money(item.amount)}</td><td><span className={styles.status}>{ORDER_STATUS[item.status] ?? item.status}</span></td><td><button className="button button-secondary" disabled={busy} onClick={() => run(() => approvePurchaseOrderAction(item.id), "Pedido aprovado.")}>Aprovar pedido</button></td></tr>)}
        {workspace.contracts.filter((item) => ["DRAFT", "UNDER_REVIEW", "IN_APPROVAL", "APPROVED"].includes(item.status)).map((item) => <tr key={`contract-${item.id}`}><td>Contrato</td><td><span className={styles.recordTitle}>{item.number}</span><span className={styles.recordMeta}>{item.title} · {item.supplier}</span></td><td>{money(item.currentAmount)}</td><td><span className={styles.status}>{CONTRACT_STATUS[item.status] ?? item.status}</span></td><td><button className="button button-secondary" disabled={busy} onClick={() => advanceContract(item.id, item.status)}>{item.status === "APPROVED" ? "Ativar contrato" : "Avançar"}</button></td></tr>)}
        ...
        {workspace.orders.filter((item) => ["DRAFT", "IN_APPROVAL"].includes(item.status)).length === 0 && workspace.contracts.filter((item) => ["DRAFT", "UNDER_REVIEW", "IN_APPROVAL", "APPROVED"].includes(item.status)).length === 0 && workspace.contracts.flatMap((contract) => contract.amendments).filter((item) => item.status === "DRAFT").length === 0 && <tr><td colSpan={5} className={styles.empty}>Nenhum pedido, contrato ou aditivo aguardando ação.</td></tr>}
      </tbody></table></div>
    </section>
  );
}