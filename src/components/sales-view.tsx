...
type Area = "visao" | "estoque" | "propostas" | "reservas" | "vendas" | "comissoes" | "posvenda";
const areas: { key: Area; label: string }[] = [
  ...
  { key: "reservas", label: "Reservas" }, { key: "vendas", label: "Vendas e Contratos" }, { key: "comissoes", label: "Comissões" }, { key: "posvenda", label: "Entrega e Pós-venda" },
];
...
export function SalesView({ workspace }: { workspace: SalesWorkspaceView }) {
  ...
  return <div className="view-stack">
    ...
    {area === "vendas" && <article className="panel"><div className="panel-heading"><div><span className="eyebrow">EVENTO COMERCIAL</span><h2>Vendas, contrato e plano de pagamento</h2></div></div><div className="data-table-scroll"><table className="data-table"><thead><tr><th>Contrato</th><th>Unidade</th><th>Compradores</th><th>Corretor</th><th>Preço vendido</th><th>Desconto</th><th>Parcelas ativas</th><th>Recebido</th><th>Situação</th></tr></thead><tbody>{workspace.sales.map((item) => <tr key={item.id}><td><strong>{item.contractNumber ?? "—"}</strong></td><td>{item.unit}</td><td>{item.buyers.join(", ")}</td><td>{item.broker ?? "—"}</td><td>{brl.format(item.soldPrice)}</td><td>{brl.format(item.discountAmount)}</td><td>{item.installments}</td><td>{brl.format(item.received)}</td><td><Status value={item.status} /></td></tr>)}</tbody></table></div></article>}
