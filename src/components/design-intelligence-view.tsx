...
type DesignTab = "dashboard" | "viewer" | "achados" | "value" | "diff";
...
export function DesignIntelligenceView({ initialWorkspace, onWorkspaceChange, onAskAI }: { initialWorkspace: DesignWorkspaceView; onWorkspaceChange: (workspace: DesignWorkspaceView) => void; onAskAI: (prompt: string) => void }) {
  ...
  const [selectedFindingId, setSelectedFindingId] = useState(initialWorkspace?.achados?.[0]?.id ?? "");
  ...
  const selectedFinding = workspace?.achados?.find((achado) => achado.id === selectedFindingId) ?? workspace?.achados?.[0];
  ...
  async function submitFinding(formData: FormData) {
    ...
    setBusy(false);
    if (!response.ok) { setError(response.error); return; }
    replaceWorkspace(response.data); setDraftRegion(null); setAnnotating(false); setTab("achados");
  }
  ...
  const tabs: Array<{ key: DesignTab; label: string; icon: typeof Gauge; count?: number }> = [
    ...
    { key: "achados", label: "Apontamentos", icon: Pin, count: workspace.summary.openAchados },
  ...
  return <div className="design-module">
    <header className="design-hero">
      ...
      <div className="design-hero-actions"><button className="button button-secondary" disabled={busy} onClick={() => setPackageOpen(true)}><PackagePlus size={15} /> Novo pacote</button><button className="button button-secondary" onClick={() => setRevisionOpen(true)}><GitCompareArrows size={15} /> Nova revisão</button><button className="button button-secondary" disabled={busy} onClick={downloadReport}><FileDown size={15} /> Design Review Report</button><button className="button button-secondary" onClick={() => onAskAI("Analise este projeto e apresente os achados com evidências.")}><Sparkles size={15} /> Analisar com REDE AI</button><button className="button button-primary" onClick={() => setUploadOpen(true)}><Upload size={15} /> Enviar projetos</button></div>
    </header>
    ...
    {tab === "dashboard" && <div className="design-dashboard">
      <section className="design-metric-grid">
        ...
        <DesignMetricCard label="Achados abertos" value={String(workspace.summary.openAchados)} meta={`${workspace.summary.criticalAchados} crítico(s)`} icon={AlertTriangle} tone={workspace.summary.criticalAchados ? "critical" : workspace.summary.openAchados ? "warning" : "positive"} />
      ...
      <section className="design-three-column">
        ...
        <article className="design-panel"><header><div><span className="eyebrow">PRINCIPAIS ACHADOS</span><h3>Prioridades da revisão</h3></div><button className="icon-button" onClick={() => setTab("achados")}><Filter size={15} /></button></header><div className="design-compact-list">{workspace.achados.slice(0, 5).map((achado) => <button key={achado.id} onClick={() => { setSelectedFindingId(achado.id); setTab("achados"); }}><span className={`design-severity ${severityTone(achado.severity)}`}>{achado.severity}</span><span><strong>{achado.title}</strong><small>{achado.discipline} · {confidenceLabel(achado.confidence)}</small></span><ChevronRight size={14} /></button>)}</div></article>
