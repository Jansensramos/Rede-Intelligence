...
export function DesignIntelligenceView({ initialWorkspace, onWorkspaceChange, onAskAI, canMutate = true }: { initialWorkspace: DesignWorkspaceView; onWorkspaceChange: (workspace: DesignWorkspaceView) => void; onAskAI: (prompt: string) => void; canMutate?: boolean }) {
  ...
  async function submitPackage(formData: FormData) {
    setBusy(true); setError("");
    const response = await createDesignPackageAction({
      projectId: workspace.package.projectId,
      name: String(formData.get("name")),
      description: String(formData.get("description") ?? ""),
      template: String(formData.get("template")) as "RESIDENTIAL_VERTICAL" | "RESIDENTIAL_HORIZONTAL" | "MIXED_USE" | "COMMERCIAL" | "LOGISTICS" | "INDUSTRIAL" | "URBANIZATION" | "CUSTOM",
      reviewMode: String(formData.get("reviewMode")) as "EXECUTIVE_REVIEW" | "ARCHITECTURAL_REVIEW" | "URBAN_REVIEW" | "PRODUCT_REVIEW" | "EFFICIENCY_REVIEW" | "COST_REVIEW" | "VALUE_ENGINEERING" | "COMMERCIAL_REVIEW" | "CONSTRUCTABILITY_REVIEW" | "BIM_COORDINATION" | "INVESTOR_REVIEW" | "FULL_REVIEW",
    });
    setBusy(false);
    if (!response.ok) { setError(response.error); return; }
    replaceWorkspace(response.data);
    setSelectedFileId("");
    setSelectedSheetId("");
  ...
  return <div className="design-module">
    <header className="design-hero">
      <div><span className="eyebrow">INTELIGÊNCIA DE PROJETOS REDE · {workspace.package.template.replaceAll("_", " ")}</span><h2>{workspace.package.name}</h2><p>{workspace.revision.label} · revisão versionada · análise preliminar com proveniência</p></div>
      <div className="design-hero-actions"><button className="button button-secondary" disabled={busy || !canMutate} onClick={() => setPackageOpen(true)}><PackagePlus size={15} /> Novo pacote</button><button className="button button-secondary" disabled={!canMutate} onClick={() => setRevisionOpen(true)}><GitCompareArrows size={15} /> Nova revisão</button><button className="button button-secondary" disabled={busy} onClick={downloadReport}><FileDown size={15} /> Design Review Report</button><button className="button button-secondary" onClick={() => onAskAI("Analise este projeto e apresente os findings com evidências.")}><Sparkles size={15} /> Analisar com REDE AI</button><button className="button button-primary" disabled={!canMutate} onClick={() => setUploadOpen(true)}><Upload size={15} /> Enviar projetos</button></div>
    </header>
