...
export interface EngineeringExceptionSignals {
  budgeted: number;
  contracted: number;
  measured: number;
  noEvidence: Array<{ id: string; description: string; amount: number }>;
  criticalItems: Array<{ id: string; observedCondition: string; risk: string; technicalResponsibleId: string | null }>;
  proposals: Array<{ id: string; name: string; reviewedAt: Date | null; createdById: string }>;
}

/** Fatos 9M viram ações calculadas; ao cessar o fato, o ID deixa naturalmente o read model 9L. */
export function buildEngineeringExceptions(ctx: ExceptionTenantContext, signals: EngineeringExceptionSignals, referenceDate: Date): ExecutiveException[] {
  const result: ExecutiveException[] = [];
  for (const item of signals.noEvidence) result.push({ id: buildExceptionId(ctx.organizationId, "operations", "budget_line_without_evidence", item.id), organizationId: ctx.organizationId, economicGroupId: ctx.economicGroupId, companyId: ctx.companyId, projectId: ctx.projectId, projectName: ctx.projectName, domain: "operations", type: "budget_line_without_evidence", title: `Item sem evidência: ${item.description}`, summary: "Preço confiável ausente; a aprovação permanece bloqueada.", severity: "ACAO_NECESSARIA", impact: { financial: item.amount }, materialityValue: item.amount, dueDate: null, confidence: ALTA, source: "REDE", occurredAt: referenceDate.toISOString(), href: "/engenharia-obra?f=engenharia&e=orcamento", reason: "Linha obrigatória classificada deterministicamente como SEM EVIDÊNCIA.", status: "ABERTA", evidence: [item.id] });
  for (const item of signals.criticalItems) result.push({ id: buildExceptionId(ctx.organizationId, "operations", "critical_engineering_opinion", item.id), organizationId: ctx.organizationId, economicGroupId: ctx.economicGroupId, companyId: ctx.companyId, projectId: ctx.projectId, projectName: ctx.projectName, domain: "operations", type: "critical_engineering_opinion", title: "Parecer Técnico crítico pendente", summary: `${item.observedCondition} · ${item.risk}`, severity: "CRITICO", impact: { schedule: true }, materialityValue: null, dueDate: null, confidence: ALTA, source: "REDE", occurredAt: referenceDate.toISOString(), href: "/engenharia-obra?f=engenharia&e=parecer", reason: "Severidade CRITICAL registrada no item e ainda não validada.", responsibleId: item.technicalResponsibleId ?? undefined, status: "ABERTA", evidence: [item.id] });
  for (const proposal of signals.proposals) result.push({ id: buildExceptionId(ctx.organizationId, "operations", "smart_budget_review_pending", proposal.id), organizationId: ctx.organizationId, economicGroupId: ctx.economicGroupId, companyId: ctx.companyId, projectId: ctx.projectId, projectName: ctx.projectName, domain: "operations", type: "smart_budget_review_pending", title: `Revisão aguardando decisão: ${proposal.name}`, summary: "A proposta não altera o Orçamento 9A até aprovação humana.", severity: "DECISAO", impact: {}, materialityValue: null, dueDate: null, confidence: ALTA, source: "REDE", occurredAt: (proposal.reviewedAt ?? referenceDate).toISOString(), href: "/engenharia-obra?f=engenharia&e=orcamento", reason: "Proposta em status REVIEW.", responsibleId: proposal.createdById, status: "ABERTA", evidence: [proposal.id] });
  if (signals.budgeted > 0 && signals.contracted > signals.budgeted * 1.05) { const delta = signals.contracted - signals.budgeted; result.push({ id: buildExceptionId(ctx.organizationId, "operations", "contracted_above_budget", ctx.projectId), organizationId: ctx.organizationId, economicGroupId: ctx.economicGroupId, companyId: ctx.companyId, projectId: ctx.projectId, projectName: ctx.projectName, domain: "operations", type: "contracted_above_budget", title: "Contratação acima do orçamento aprovado", summary: `Contratado R$ ${signals.contracted.toLocaleString("pt-BR")} · Orçado R$ ${signals.budgeted.toLocaleString("pt-BR")}`, severity: "ACAO_NECESSARIA", impact: { financial: delta }, materialityValue: delta, dueDate: null, confidence: ALTA, source: "REDE", occurredAt: referenceDate.toISOString(), href: "/engenharia-obra?f=engenharia&e=ciclo", reason: "Valor contratado supera o orçamento aprovado em mais de 5%.", status: "ABERTA", evidence: [ctx.projectId] }); }
  if (signals.contracted > 0 && signals.measured > signals.contracted * 1.005) { const delta = signals.measured - signals.contracted; result.push({ id: buildExceptionId(ctx.organizationId, "operations", "measurement_above_contract", ctx.projectId), organizationId: ctx.organizationId, economicGroupId: ctx.economicGroupId, companyId: ctx.companyId, projectId: ctx.projectId, projectName: ctx.projectName, domain: "operations", type: "measurement_above_contract", title: "Medição acima do contratado", summary: `Medido R$ ${signals.measured.toLocaleString("pt-BR")} · Contratado R$ ${signals.contracted.toLocaleString("pt-BR")}`, severity: "CRITICO", impact: { financial: delta }, materialityValue: delta, dueDate: null, confidence: ALTA, source: "REDE", occurredAt: referenceDate.toISOString(), href: "/engenharia-obra?f=engenharia&e=ciclo", reason: "Medições certificadas superam o total contratado.", status: "ABERTA", evidence: [ctx.projectId] }); }
  return result;
}
...
export function buildCapitalExceptions(
  ...
  for (const covenant of covenants) {
    ...
    exceptions.push({
      ...
      title: covenant.status === "BREACHED" ? `Cláusula financeira violada: ${covenant.code}` : `Cláusula financeira ${covenant.code} em atenção`,
      ...
      occurredAt: referenceDate.toISOString(),
      href: "/capital-funding",
      reason: covenant.status === "BREACHED" ? "Último teste da cláusula financeira resultou em violação." : "Último teste da cláusula financeira está fora da margem de segurança.",
      status: "ABERTA",
  ...
  for (const proposal of proposalsAwaitingDecision) {
    ...
    exceptions.push({
      ...
      title: `Proposta de financiamento aguardando decisão: ${proposal.code}`,
