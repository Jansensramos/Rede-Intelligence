...
export function planAIIntent(question: string): AIIntentPlan {
  ...
  if (/matricula|iptu|diligencia imobiliaria|cadeia dominial|terreno.*juridic/.test(text)) return { task: "ANALYSIS", calls: [{ name: "getPropertyDueDiligence", arguments: {} }, { name: "getLegalReadiness", arguments: {} }], responseMode: mode };
  if (/licenca|obrigacao legal|prazo juridic|vencimento legal|condicionante/.test(text)) return { task: "ANALYSIS", calls: [{ name: "getLegalDeadlines", arguments: {} }, { name: "getLegalReadiness", arguments: {} }], responseMode: mode };
