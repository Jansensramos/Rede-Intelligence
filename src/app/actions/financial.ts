...
const write = () => requireDomainWriteContext("FINANCIAL_READ", "FINANCIAL_WRITE");
const approve = () => requireDomainApprovalContext("FINANCIAL_READ", "FINANCIAL_APPROVE");
import {
...
export async function transitionPayableInstallmentAction(installmentId: string, to: Parameters<typeof transitionPayableInstallment>[2], reason?: string) {
  const context = to === "APROVADA" ? await approve() : await write();
