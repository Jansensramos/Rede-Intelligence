...
export default async function LocalReadinessPage() {
  ...
  catch (error) { if (error instanceof ReadinessAccessError) redirect("/acesso-negado"); return <section className="panel"><h1>Prontidão local indisponível</h1><p>Não foi possível verificar os dados. Nenhum critério foi aprovado.</p></section>; }
  ...
  return <div className="view-stack">
    <section className="panel">
      ...
      <p>Versão: {report.identity.commit ?? "não identificada"} · Compilação: {report.identity.build ?? "não identificada"} · Atualizações do banco: {report.migrationCount}</p>
    ...
    <section className="panel">
      ...
      <p>Falhas pendentes: {report.metrics.deadLetters} · Quarentena pendente: {report.metrics.quarantine} · Atraso da fila: {report.metrics.overdueSeconds}s</p>
      ...
      <p>Situação dos provedores externos: {report.providerHealth === "REAL_NOT_VERIFIED" ? "não verificada por esta checagem local" : report.providerHealth}.</p>
