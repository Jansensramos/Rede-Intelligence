...
describe("SecretProvider", () => {
  ...
  it("proíbe environment provider em produção", () => {
    expect(() => createSecretProvider({ NODE_ENV: "production", DATABASE_URL: "db", SESSION_COOKIE_NAME: "rede", TRUSTED_PROXY_HOPS: 0, STORAGE_PROVIDER: "s3", STORAGE_FORCE_PATH_STYLE: "false", STORAGE_SIGNED_URL_TTL_SECONDS: 300, MALWARE_SCANNER_PROVIDER: "external", SECRET_PROVIDER: "environment", KMS_PROVIDER: "external", ALERTING_PROVIDER: "local", WORKER_CONCURRENCY: 1, WORKER_POLL_MS: 100, WORKER_LEASE_MS: 5000, WORKER_JOB_TIMEOUT_MS: 1000, AI_GATEWAY_PROVIDER_MODE: "disabled" })).toThrow(/proibido/);
  });
